...
export class Main {
    ...
    static async start(levelId, lang = 'ro') {
        ...
        function showAlert(msg, showButton = true, btnText = t('quiz.ok'), onClose = null) {
            ...
            if (showButton) {
                alertDiv.innerHTML = `<div style='margin-bottom:18px;'>${msg}</div>
                                      <button id='quiz-alert-btn'>${btnText}</button>`;
            } else {
        ...
        function loop(timestamp) {
            ...
            for (const coin of coins) {
                if (coin.checkCollected(player)) {
                    Main.addCoin();
                    const sfx = new window.Audio('assets/music/collect.mp3');
                    sfx.volume = 0.35;
                    sfx.play();
                }
            }
                ...
                if (atEdge && !triedAdvance) {
                    ...
                    const quiz = new QuizSystem(
                        ...
                        () => {
                            ...
                            edgePopupText  = Main._lang === 'ro'
                                ? 'Ai răspuns greșit! Încearcă din nou.'
